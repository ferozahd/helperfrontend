...
- To make a lot of our changes, we've followed the instructions from here (minus the `colors` and `font-sizes`): https://tailwindcss.com/docs/upgrading-to-v2
  - For the colors, the only change that we made, is the fact that we've added all Tailwind CSS colors to our `tailwind.config.js` files, and inside our product, all `{type}-gray-{number}` classes were renamed to `{type}-gray-{number}`
  - After that, we've changed `{type}-gray-{number}` to `{type}-gray-{lower-number}`, i.e. (`100` became `50`, `200` became `100`, ..., `900` became `800`)
    - You can achieve this, by search in your whole project for `gray-100` and replace it with `gray-50`
    - Then, you search in your whole project for `gray-200` and replace it with `gray-100`
    - Then, you search in your whole project for `gray-300` and replace it with `gray-200`
    - Then, you search in your whole project for `gray-400` and replace it with `gray-300`
    - Then, you search in your whole project for `gray-500` and replace it with `gray-400`
    - Then, you search in your whole project for `gray-600` and replace it with `gray-500`
    - Then, you search in your whole project for `gray-700` and replace it with `gray-600`
    - Then, you search in your whole project for `gray-800` and replace it with `gray-700`
    - Then, you search in your whole project for `gray-900` and replace it with `gray-800`
